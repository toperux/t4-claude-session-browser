...
main() {
    ...
    # Same block as installer/install.sh; keep them in step.
    version="${CSB_VERSION:-}"
    version="${version#v}"
    if [ -z "$version" ]; then
    ...
    (cd "$tmp" && grep " $file\$" SHA256SUMS | $sum -c -)
    ...
    # session's PATH. $dir is user-supplied, so the line is rewritten by dropping
    # and reprinting it rather than through a sed replacement, where `&`, `|` or
    # `\` in the path would be taken as sed syntax. Older releases ship no
    # desktop entry or icon - then there is none.
    if [ "$os" = Linux ] && [ -f "$tmp/csb.desktop" ] && [ -f "$tmp/csb.png" ]; then
        share="${XDG_DATA_HOME:-$HOME/.local/share}"
        mkdir -p "$share/applications" "$share/icons/hicolor/256x256/apps"
        { grep -v '^Exec=' "$tmp/csb.desktop"; printf 'Exec="%s/csb" gui\n' "$dir"; } > "$share/applications/csb.desktop"
